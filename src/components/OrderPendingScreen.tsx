...
export function OrderPendingScreen() {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      if (Date.now() - startedAt > PENDING_TIMEOUT_MS) setOrderPhase("recovery")
    }, POLL_INTERVAL_MS)
    return () => {
  ...
  return (
    <div className="app-screen">
      <ReferralDebugStrip />
      <main className="center-card-screen">
        <div className="center-card text-center space-y-6">
          {!isRecovery && (
            <div className="flex justify-center">
              <div className="w-16 h-16 border-4 rounded-full animate-spin" style={{ borderColor: `${lineColor}30`, borderTopColor: lineColor }} />
            </div>
          )}
          ...
          <div className="space-y-2">
            {isRecovery ? (
              <>
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">Your order transaction was sent. Do not open another same-direction trade until you check GMX or Arbiscan.</p>
              </>
            ) : (
              <>
                <h2 className="text-xl font-bold">Opening your {marketInfo?.symbol} {directionLabel} trade...</h2>
                <p className="text-sm text-muted-foreground leading-relaxed">GMX is confirming your trade. This usually takes a few seconds.{activePosition?.openTxHash ? " A keeper is executing your order." : ""}</p>
              </>
            )}
          </div>

          <div className="rounded-xl bg-[#12121a] border border-[#1e1e30] p-4 space-y-2 text-left">
            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Market</span><span className="font-semibold">{marketInfo?.icon} {activePosition?.marketKey}</span></div>
            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Direction</span><span className={`font-semibold ${isLong ? "text-[#22c55e]" : "text-[#ef4444]"}`}>{directionLabel}</span></div>
            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Risk</span><span className="font-mono tabular-nums">${activePosition?.riskUsd.toFixed(2)}</span></div>
          </div>
          ...
          {!isRecovery && activePosition?.openTxHash && <a href={arbiscanTxLink(activePosition.openTxHash)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-[#418cf5]/70 hover:text-[#418cf5] transition-colors">View transaction &rarr;</a>}

          <details className="rounded-xl bg-[#418cf5]/5 border border-[#418cf5]/10 p-3 text-[11px] text-muted-foreground/80 leading-relaxed text-left">
            <summary className="cursor-pointer">Details</summary>
            <p className="pt-2">GMX uses keepers to execute orders with fresh prices. EasyGMX keeps checking until your position appears or you leave this screen.</p>
            {isRecovery && <p className="pt-2 text-amber-500/80">Refreshing the page will return you to the home screen. Your transaction may still execute on GMX — check Arbiscan or GMX directly.</p>}
          </details>
        </div>
      </main>
    </div>
